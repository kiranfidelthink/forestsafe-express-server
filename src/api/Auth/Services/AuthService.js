...
exports.resetPasswordMail = async (data) => {
  let response , result;
  const { email } = data;
  await User.findOne({ emailAddress: email }).then(async (user) => {
    if (!user) {
      console.log('User not found');
    } else {
      if (email === user.emailAddress) {
        result= authentication.generateToken(user)
        var mailOptions = {
          to: email,
          ...
          html:'<div style="font-size: 16px;">'+
            ...
            `<a href='https://crew.forestsafe.co.nz/password/reset/${result.token}?email=${email}'>https://crew.forestsafe.co.nz/password/reset/${result.token}?email=${email}</a>` +
